...
def parse_args():
    ...
    ## DPO
    parser.add_argument("--train_method", type=str, choices=['sft', 'dpo', 'dpo_improved'], required=True, 
    help="Specify the method to use: sft (Supervised Fine-Tuning), dpo (Direct Preference Optimization), or dpo_improved."
)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if args.train_method == 'dpo' or "dpo_improved":
                ...
                elif args.train_method == 'dpo_improved':
                    # model_pred and ref_pred will be (2 * LBS) x 4 x latent_spatial_dim x latent_spatial_dim
                    # losses are both 2 * LBS
                    # 1st half of tensors is preferred (y_w), second half is unpreferred
                    model_losses = (model_pred - target).pow(2).mean(dim=[1,2,3])
