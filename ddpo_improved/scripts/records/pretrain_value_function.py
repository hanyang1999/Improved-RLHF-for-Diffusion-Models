...
def main(_):
    ...
    def _train_value_func_with_regularization(value_function, samples_batched, accelerator, config):
        ...
        "prompt_embeds_value"
        """
        batch_state_original = samples_batched["latents"] # batchsize * timesteps *latent_dims
        
        # Combine first two dimensions of batch_state_original into 1
        new_first_dim = batch_state_original.shape[0] * batch_state_original.shape[1] 
        batch_state = batch_state_original.reshape(new_first_dim, *batch_state_original.shape[2:])
        
        # Same for batch_timestep, leading to (batchsize * timesteps,1) tensor (already randomly permuted)
        ...
        batch_txt_emb_value = samples_batched["prompt_embeds_value"] # batch_size * token_embed_dim (768)
        
        # batch_size * timestep
        batch_prompt = samples_batched["prompt"]

        # project timesteps to 0-50
        batch_timestep = (batch_timestep - 1) * config.sample.num_steps / 1000
        batch_timestep = batch_timestep.int() 

        # duplicate txt embeddings
        repeats = int(new_first_dim / batch_txt_emb_value.shape[0])
        repeated_txt_emb_value = []
        for i in range(batch_txt_emb_value.shape[0]):
            row = batch_txt_emb_value[i].unsqueeze(0)
        ...
        batch_txt_emb_value = torch.cat(repeated_txt_emb_value, dim=0) # (batch_size*num_inference_steps,token_embed_dim)
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        samples = {
            k: torch.cat([s[k] for s in samples]) if isinstance(samples[0][k], torch.Tensor)
            else tuple(item for s in samples for item in s[k]) if isinstance(samples[0][k], tuple)
            else None  
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            samples = {
                k: torch.stack([v[i] for i in perm]) if isinstance(v, torch.Tensor)
                else tuple(v[i] for i in perm.tolist()) if isinstance(v, tuple)
                else v[perm]  
        ...
        accelerator.log({"value_loss": avg_value_loss}, step=global_step)
        global_step += 1

        if epoch != 0 and epoch % config.save_freq == 0 and accelerator.is_main_process:
            accelerator.save_state()
            output_dir = os.path.join(accelerator.project_dir, "checkpoints")
            output_dir = os.path.join(output_dir, f"checkpoint_{accelerator.save_iteration - 1 }")
            output_dir = os.path.join(output_dir, "value_func_pretrain.pth")
            torch.save(value_function.state_dict(), output_dir)
