...
from ddpo_pytorch.diffusers_patch.pipeline_with_logprob import pipeline_with_logprob, pipeline_with_logprob_regularizedReward, pipeline_continuous_regularizedReward
...
def main(_):
    ...
    state_dict = torch.load('/home/hanyang/ddpo/logs/ddpo_compressibility_pretrain_2024.05.17_12.08.37/checkpoints/checkpoint_18/value_func_pretrain.pth')
    ...
    for epoch in range(first_epoch, config.num_epochs):
        ...
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            if config.use_regularization:
                ...
                samples["traj_regularization_cumsum_inv"] = samples["traj_regularization_cumsum_inv"][torch.arange(total_batch_size, device=accelerator.device)[:, None], perms]
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(unet):
                        ...
                        current_latent = sample["latents"][:,j].clone().requires_grad_(True)
                        value_func = value_function(current_latent, sample["prompt_embeds"][:,j], scaled_timestep)
                        
                        value_func.backward(torch.ones_like(value_func))
                        latent_grad = current_latent.grad # batch_size * latents_dims
                        ...
                        CTD_error = sample["rewards"][:] + sample["traj_regularization_cumsum_inv"][:,j] - value_func 
                        # + C * (actual_noise_predictor-noise_predictor)  # batch_size * latents_dims
                        advantages = CTD_error 
                        # shall we scale with mean and std like for DDPO original implementation?
