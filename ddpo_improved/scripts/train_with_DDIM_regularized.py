...
from ddpo_pytorch.diffusers_patch.pipeline_with_logprob import pipeline_DDIM,pipeline_with_logprob, pipeline_with_logprob_regularizedReward
...
def main(_):
    ...
    # original_unet.to(accelerator.device, dtype=inference_dtype)
    ...
    #     # 检查两个状态字典的键是否相同（这应该总是成立的，除非模型架构不同）
    ...
    # # 比较模型
    # compare_models(original_unet, unet)
    # bp()
    ...
    for epoch in range(first_epoch, config.num_epochs):
            ...
            with autocast():
                if config.use_regularization:
                    images, _, latents, log_probs, all_regularization_terms = pipeline_with_logprob_regularizedReward(
                        ...
                        eta=get_decayed_value(epoch, config),
                        output_type="pt",
        ...
        for sample in tqdm(
            ...
            if config.use_regularization:
                discount_factor = (1 + get_decayed_value(epoch, config)**2) / (2 * get_decayed_value(epoch, config))
                sample["rewards_regularized"] = sample["rewards"] - config.penalty_constant / 2.0 * discount_factor**2 * sample["regularization"] 
