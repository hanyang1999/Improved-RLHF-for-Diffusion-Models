...
def get_config():
    ...
    config.resume_from = ""
    # "/home/hanyang/diffusion_alignment/ddpo_improved/logs/ddpo_aesthetic_no_regularization_eta=1.0_lr=0.0015_2024.10.30_11.27.43/checkpoints/checkpoint_3"
    ...
    ###### Using Regularization in RL objective #####
    config.use_regularization = False
    
    config.penalty_constant = 10 ** -2
    ...
    ''' Decay parameters for sampling '''
    sample.decay = decay = ml_collections.ConfigDict()
    decay.type = "constant"  # Options: "constant", "linear", "cosine", "exponential", "step", "polynomial"
    decay.initial_value = sample.eta

    # Decay-specific configurations within the sample part
    decay.linear = ml_collections.ConfigDict()
    decay.linear.decay_rate = 0.005

    decay.cosine = ml_collections.ConfigDict()
    decay.cosine.max_steps = 2 * config.num_epochs

    decay.exponential = ml_collections.ConfigDict()
    decay.exponential.decay_rate = 0.9

    decay.step = ml_collections.ConfigDict()
    decay.step.decay_rate = 0.5
    decay.step.decay_steps = 5

    decay.polynomial = ml_collections.ConfigDict()
    decay.polynomial.max_steps = 2 * config.num_epochs
