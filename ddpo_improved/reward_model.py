...
class ValueMulti(nn.Module):
  ...
  def forward(self, img, txt_emb, t, prompt = None):
    x = img.view(img.shape[0], -1)
...
class ValueMulti_continuous(nn.Module):
  def __init__(self, T, value_multi_model, reward_model):
    ...
    self.T = T
    self.value_multi_model = value_multi_model
    self.reward_model = reward_model

  def forward(self, img, text_emb, t):
    return (t / self.T) * self.value_multi_model(img, text_emb) + (1 - t / self.T) * self.reward_model(img, text_emb)

class ValueMulti_continuous_v2(nn.Module):
  """ValueMulti_continuous_v2."""

  def __init__(self, num_steps, img_shape, reward_model):
    super(ValueMulti_continuous_v2, self).__init__()
    self.lin1 = ConditionalLinear(int(np.prod(img_shape)) + 768, 256, num_steps)
    self.lin2 = ConditionalLinear(256, 256, num_steps)
    self.lin3 = ConditionalLinear(256, 256, num_steps)
    self.lin4 = nn.Linear(256, 1)
  ...
  def forward(self, img, txt_emb, t, prompt, promt_metadata = None):
    x = img.view(img.shape[0], -1)
    x = torch.cat([x, txt_emb], dim=1)
    # x = torch.cat([x, txt_emb], dim=1)
    x = F.relu(self.lin1(x, t))
    ...
    x = F.relu(self.lin3(x, t))
    x = self.lin4(x)
    reward, _ = self.reward_model(img, prompt, promt_metadata)
    x = (t / self.T) * x + (1 - t / self.T) * reward
    return self.lin4(x)
